...
exports.enablePowershellRemoting = edge.func('ps', function () {/*
  ...
  try
  {
    ...
    # Filter computerNames that are already enabled
    $computerNames = ($computerNames.split(",") | Where { -Not (Test-PsRemoting -ComputerName $_ -Credential $cred) } ) -Join ","
...
exports.execAtRemote = edge.func('ps', function () {/*
  ...
  Function Query-RemoteSession
  {
    ...
    if ($?) {
      ...
      $command = "& { .\bin\psexec.exe $('\\' + $computerName) -u $username -p $password -s qwinsta.exe } > ${env:TEMP}\pstools-qwinsta-${hosts}.txt"
  ...
  try
  {
    ...
    ForEach ($computerName in $computerNames.split(",")) {
      ...
      #$sessionId, $sessionState, $sessionName = Query-RemoteSession -hosts $computerName -username $username -password $password -psSession $psSession | Parse-RemoteSession
      if ($sessionId -eq $Null) {
        ...
        if ($?) {
          ...
          $p = Start-Process .\bin\psexec.exe -ArgumentList ("\\$computerName", '-u', $username, '-p', $password, '-s', 'cmd', '/c', 'reg add "HKLM\SYSTEM\CurrentControlSet\Control\Terminal Server" /f /v fDenyTSConnections /t REG_DWORD /d 0 && netsh advfirewall firewall set rule group="@FirewallAPI.dll,-28752" new enable=yes') -PassThru -WindowStyle Minimized
          if (-Not $?) {
        ...
        $p = Start-Process .\bin\wfreerdp.exe -ArgumentList ("/v:$computerName", "/u:$username", "/p:$password", '-themes', '-wallpaper', '-grab-keyboard') -PassThru -WindowStyle Minimized
        if (-Not $?) {
      ...
      if ($sessionState -ne 'Disc') {
        ...
        #  }
        #} else {
        #  $p = Start-Process .\bin\psexec.exe -ArgumentList ("\\$computerName", '-u', $username, '-p', $password, '-i', $sessionId, '-h', 'tsdiscon.exe') -PassThru -WindowStyle Minimized
        #  if (-Not $?) {
      ...
      # Ensuring psexec.exe exists on remote host
      $rPsExecExists = Invoke-Command -Session $psSession -ScriptBlock { Get-Command psexec.exe -ErrorAction SilentlyContinue }
      if ($?) {
        if (-Not $rPsExecExists) {
          $remoteDestination = Join-Path -Path ('\\' + $computerName) -ChildPath "c$\Windows\System32\psexec.exe"
          ...
          Copy-Item ".\bin\psexec.exe" -Destination $remoteDestination
          if ($DriveName -ne $Null) { Remove-PSDrive $DriveName -ErrorAction SilentlyContinue }
        }
      }
      ...
      $argumentList = "\\$computerName", '-u', $username, '-p', $password, '-i', $sessionId, '-h', $inputFromJS.cmd

      # http://stackoverflow.com/questions/10666101/powershell-lastexitcode-0-but-false-redirecting-stderr-to-stdout-gives-nat/
      Invoke-Command -Session $psSession -ScriptBlock { if (Get-Command psexec.exe -ErrorAction SilentlyContinue) { psexec.exe -accepteula ($using:argumentList + $using:params) 2>&1 | %{ "$_" } } else { echo "Could not locate PsExec.exe on remote host"; $LastExitCode = -1 } }
      if ($?) {
        $rExitCode = Invoke-Command -Session $psSession -ScriptBlock { $LastExitCode }
        if ($rExitCode -ne 0) {
      ...
      # Remove psSession before next iteration
