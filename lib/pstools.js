...
exports.copyFileToRemote = edge.func('ps', function () {/*
  ...
  try
  {
    ...
    if (($inputFromJS.sources -is [string]) -Or ($inputFromJS.sources -is [array])) {
      ...
      $remoteDestination = Join-Path -Path ('\\' + $inputFromJS.host) -ChildPath $destination
...
exports.copyFileFromRemote = edge.func('ps', function () {/*
  ...
  try
  {
    ...
    if (($inputFromJS.sources -is [string]) -Or ($inputFromJS.sources -is [array])) {
      ...
      $remoteSources = $sources | ForEach-Object { Join-Path -Path ('\\' + $inputFromJS.host) -ChildPath $_ }
...
exports.execAtRemote = edge.func('ps', function () {/*
  ...
  Function Query-RemoteSession
  {
    ...
    $p = Start-Process powershell -ArgumentList ('-Command', $command) -PassThru -WindowStyle Minimized
    if (-Not $?) {
    ...
    $sessionSelect = Get-Content "${env:TEMP}\pstools-qwinsta-${hosts}.txt" | Select-String "([\w-#]*)\s+${username}\s+(\w+)\s+(\w+)"
  ...
  try
  {
    ...
    if ($sessionId -eq $Null) {
      ...
      $p = Start-Process .\bin\psexec.exe -ArgumentList ("\\$hosts", '-u', $username, '-p', $password, '-s', 'cmd', '/c', 'reg add "hklm\system\currentcontrolset\control\terminal server" /f /v fDenyTSConnections /t REG_DWORD /d 0 && netsh advfirewall firewall set rule group="@FirewallAPI.dll,-28752" new enable=yes') -PassThru -WindowStyle Minimized
      if (-Not $?) {
      ...
      $p = Start-Process .\bin\wfreerdp.exe -ArgumentList ("/v:$hosts", "/u:$username", "/p:$password", '-themes', '-wallpaper', '-grab-keyboard') -PassThru -WindowStyle Minimized
      if (-Not $?) {
    ...
    # Disconnect session to prevent user inteference
      ...
      #$p = Start-Process .\bin\psexec.exe -ArgumentList ("\\$hosts", '-u', $username, '-p', $password, '-i', $sessionId, '-h', 'tsdiscon.exe') -PassThru -WindowStyle Minimized
      #if (-Not $?) {
    ...
    $argumentList = "\\$hosts", '-u', $username, '-p', $password, '-i', $sessionId, '-h', $inputFromJS.cmd
    ...
    # Workaround as psexec cannot pipe stdout when running interactively
    #Start-Process .\bin\psexec.exe -ArgumentList ("\\$hosts", '-u', $username, '-p', $password, 'cmd', '/c', 'type', "$($inputFromJS.cmd).log") -Wait -WindowStyle Minimized -RedirectStandardOutput "${env:TEMP}\pstools-log.txt"
    #$command = "& { .\bin\psexec.exe $('\\' + $hosts) -u $username -p $password cmd /c type '$($inputFromJS.cmd).log' } > ${env:TEMP}\pstools-log.txt"
    #Start-Process powershell -ArgumentList ('-Command', $command) -Wait -WindowStyle Minimized
    #Get-Content "${env:TEMP}\pstools-log.txt" | Out-String
