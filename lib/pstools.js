...
exports.execAtRemote = edge.func('ps', function () {/*
  ...
  Function Query-RemoteSession
  {
    ...
    if (-Not $?) {
      throw "Could not query for interactive winlogon sessions on remote host."
    }
  ...
  # Temporarily Here
    ...
    if ($sessionId -eq $Null) {
      ...
      if ($LastExitCode -ne 0) {
        throw "Could not start wfreerdp. ExitCode: $LastExitCode"
      }
    ...
    if ($sessionId -ne $Null) {
      ...
      throw "Could not find an existing session nor start a new one."
    }
    ...
    if ($LastExitCode -ne 0) {
      throw "Remote executable ran but did not execute correctly. ExitCode: $LastExitCode"
    }
