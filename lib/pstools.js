...
exports.execAtRemote = edge.func('ps', function () {/*
  ...
  Function Query-RemoteSession
  {
    ...
    $Null = & .\bin\PaExec.exe -accepteula $('\\' + $hosts) -u $username -p $password -s "C:\Windows\Sysnative\qwinsta.exe" | Tee-Object -Variable stdOut

    # Get session id and state
    $sessionSelect = $stdOut | Select-String "([\w-#]*)\s+${username}\s+(\w+)\s+(\w+)"
  ...
  $process = {
    ...
    & .\bin\PaExec.exe -accepteula \\$hosts -u $username -p $password cmd /c "type ${inputFromJS.cmd}.log" | Tee-Object -Variable stdOut
    echo $stdOut
  }
