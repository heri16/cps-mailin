...
exports.execAtRemote = edge.func('ps', function () {/*
  ...
  try
  {
    ...
    if ($sessionState -ne 'Disc') {
      ...
      #Invoke-Command -Session $psSession -ScriptBlock { if (Test-Path D:\psexec.exe) { D:\psexec.exe -accepteula \\$using:computerNames -u $using:username -p $using:password -i $using:sessionId -h tsdiscon.exe 2>&1 | %{ "$_" } } }
      #if ($?) {
      #  $rExitCode = Invoke-Command -Session $psSession -ScriptBlock { $LastExitCode }
      #  if ($rExitCode -ne 0) {
    ...
    Invoke-Command -Session $psSession -ScriptBlock { if (Test-Path D:\psexec.exe) { D:\psexec.exe -accepteula ($using:argumentList + $using:params) 2>&1 | %{ "$_" } } }
    if ($?) {
      $rExitCode = Invoke-Command -Session $psSession -ScriptBlock { $LastExitCode }
      if ($rExitCode -ne 0) {
